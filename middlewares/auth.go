...
func verfifyFirebaseIDToken(ctx echo.Context, auth *auth.Client) (*auth.Token, error) {
...
func FirebaseGuard() echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			jwtToken, err := verfifyFirebaseIDToken(c, authClient)
...
func FirebaseAuth() echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			authClient := c.Get("firebase").(*auth.Client)
			jwtToken, _ := verfifyFirebaseIDToken(c, authClient)
